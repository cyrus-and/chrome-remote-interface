...
function addEvent(domain, event, help) {
    var self = this;
    Chrome.prototype[domain][event] = function (handler) {
        self.on(domain + '.' + event, handler);
    };
    Chrome.prototype[domain][event].help = help;
}
...
function addCommandShorthands() {
    ...
    for (var domain in protocol.domains) {
        ...
        // TODO make dry
        var events = protocol.domains[domain].events;
        if (events) {
            for (var i = 0; i < events.length; i++) {
                var event = events[i].name;
                addEvent.call(self, domainName, event, events[i]);
            }
        }
    }
}
