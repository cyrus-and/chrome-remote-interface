...
Chrome.spawnTab = function (options, callback) {
    ...
    var request = http.get(httpOptions, function (response) {
        ...
        response.on('end', function () {
            if(data === 'Could not create new page') {
...
Chrome.closeTab = function (options, callback) {
    if (typeof options === 'function') {
        callback = options;
        return callback(new Error('Must provide target tab ID in options!'));
    }
    options = options || {};
    options.host = options.host || defaults.HOST;
    options.port = options.port || defaults.PORT;
    options.id = options.id || options.target;
    var httpOptions = {'host': options.host,
                       'port': options.port,
                       'path': '/json/close/'+options.id};
    var request = http.get(httpOptions, function (response) {
        var data = '';
        response.on('data', function (chunk) {
            data += chunk;
